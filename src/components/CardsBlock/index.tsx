...
const CardsBlock = ({ item, refs, pageHeight, index }: { item: any; refs: any; pageHeight: number; index: number }) => {
    ...
    return (
        <div id={item.description.id} ref={refs[item.description.id]}>
            <h2 className="text-[#21201F] text-[30px] font-medium mb-4 lg:text-[24px]">{item.description.name}</h2>
